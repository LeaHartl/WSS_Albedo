...
def timeseriesplot_S2stakes_2panels(fname, df_mean, nogood):
    ...
    ax[0].set_title('AWS position: in situ vs. S2 albedo')
    ax[1].set_title('Stake positions: S2 albedo')
    ax[0].legend(loc='center right', bbox_to_anchor=(1.2, 0.5),fontsize=12)
    ax[1].legend(loc='center right', bbox_to_anchor=(1.1, 0.5),fontsize=12)
    
    for a in ax:
        a.grid('both')
        a.set_ylim([0, 1])
        a.set_ylabel('Albedo', fontsize=14)
...
def countLowAlbDays(all_d, stake_data2, annual_stakes):
    ...
    for j, st in enumerate(['A', 'B', 'C', 'D', 'E', 'F', 'H']):
        ax[0].scatter(annual_stakes[st], counts_toplot[st], color=mcolors.rgb2hex(clrs[j], keep_alpha=True), label=st)
    ...
    for i, d in enumerate(dts):
        if d[0].year>2017:
            ...
            if len(tm.date0.values)>0:
                ax[1].scatter(tm.ice_ablation_mmwe/tm.period, 100*tm.img_count_extrapl/tm.period, label=d[0].strftime('%Y-%m-%d')+' - '+d[1].strftime('%Y-%m-%d'), color=clrs2[i])#, s=tm.period)
    ...
    ax[0].set_xlabel('Annual ice ablation at stakes [mm w.e.]', fontsize=14)
    ax[1].set_xlabel('Ice ablation rate at stakes [mm w.e./day]', fontsize=14)

    ax[0].set_ylabel('Low albedo at stake positions [days per year]', fontsize=14)
    ax[1].set_ylabel('Low albedo at stake positions [% of measurement period]', fontsize=14)

    ax[0].text(-1500, 0,'a', fontsize=12,
        bbox=dict(boxstyle="square,pad=0.3",fc="lightgrey", ec="grey", lw=2))

    ax[1].text(-27, 0,'b', fontsize=12,
        bbox=dict(boxstyle="square,pad=0.3",fc="lightgrey", ec="grey", lw=2))
...
ax = ax.flatten()
# ax[0].plot(data.index, ~data.SWout_Avg.isnull())
ax[0].fill_between(data.index, ~data.SWout_Avg.isnull(), color='grey', alpha=0.6)

ax[0].set_xlim(pd.to_datetime('2017-06-01'), pd.to_datetime('2024-12-31'))
ticks = ['2017-07-01', '2018-01-01', '2018-07-01','2019-01-01', '2019-07-01','2020-01-01', '2020-07-01',
         '2021-01-01', '2021-07-01', '2022-01-01', '2022-07-01', '2023-01-01', '2023-07-01', '2024-01-01', '2024-07-01']
ax[0].set_xticks(pd.to_datetime(ticks))
ax[3].set_xticklabels(ticks, rotation=45)
ax[0].set_ylim(0, 1)

colors = lrs = cm.tab10(np.linspace(0, 1, len(sts)))

for i, s in enumerate(sts):
    print(s)
    dt0 = stake_data.loc[stake_data.name == s, 'date0']
    dt1 = stake_data.loc[stake_data.name == s, 'date1']
    df = pd.DataFrame(columns=['dt0', 'dt1', 'i'])
    df['dt0'] = pd.to_datetime(dt0)
    df['dt1'] = pd.to_datetime(dt1)
    df['i'] = (i+0.8)*0.1
    ax[1].scatter(df['dt0'], df['i'], color=colors[i])
    ax[1].scatter(df['dt1'], df['i'], color=colors[i], label=s)

sts2 = all_d.dropna(subset=['name']).name.unique()
for i, s2 in enumerate(sts2):
    print(s2)
    dt = all_d.loc[all_d.name == s2]
    df = pd.DataFrame(columns=['dt','i'])
    df['dt'] = pd.to_datetime(dt.index)
    df['i'] = (i+0.8)*0.1
    ax[2].scatter(df['dt'], df['i'], s=1, color=colors[i], label=s2)


dates_glacierwide = ['20180817', '20190827', '20200915', '20210821', '20220824', '20230910', '20240907']
dates_summit22 = ['20220712', '20220717', '20220722', '20220804', '20220809', '20220814', '20220816',
                '20220821', '20220824', '20220829', '20220913', '20220923']
dates_summit18 = ['20180713', '20180718', '20180731', '20180805', '20180815', '20180817', '20180827']

ax[3].vlines(pd.to_datetime(dates_glacierwide), 0, 1, color='k', zorder=100, linewidth=2, label='glacierwide albedo')
ax[3].vlines(pd.to_datetime(dates_summit22), 0, 1, color='grey', label='WSS summit region albedo')
ax[3].vlines(pd.to_datetime(dates_summit18), 0, 1, color='grey')

ax[1].legend(loc='upper center', bbox_to_anchor=(-0.08, 1.2))
ax[2].legend(loc='upper center', bbox_to_anchor=(-0.08, 1.1))
ax[3].legend(loc='upper center', bbox_to_anchor=(-0.08, 0.8), ncol=1)

for a in ax:
    a.yaxis.set_visible(False)
    a.grid('x')

ax[0].set_title('AWS data coverage')
