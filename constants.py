...
' INITIAL CONDITIONS '
initial_snowheight_constant = 0.28 # Used 0.28 for eval run during August 2021 period. Used 0.0 otherwise. # Initial snowheight (modfied)
initial_snow_layer_heights = 0.10               # Initial thickness of snow layers
initial_glacier_height = 6 # Estimate based on 8-12 m measured (GPR) ice thickness in 2018 and losses since measurement # Initial glacier height without snowlayers  (modified)
initial_glacier_layer_heights = 0.5             # Initial thickness of glacier ice layers

initial_top_density_snowpack = 200.0   # Modified for eval run during August 2021 period, otherwise snow is set to 0. # Top density for initial snowpack
initial_bottom_density_snowpack = 350.0  # Modified for eval run during August 2021 period, otherwise snow is set to 0.  # Bottom density for initial snowpack

temperature_bottom = 269.125 #estimated value based on WSS temp strings # Lower boundary condition for initial temperature profile (K)
...
' MODEL CONSTANTS '
center_snow_transfer_function = 1             # center (50/50) when total precipitation is transferred to snow and rain
...
minimum_snowfall = 0.000001                        # minimum snowfall per time step in m which is added as new snow
...
' PHYSICAL CONSTANTS '
constant_density = 68.                         # constant density of freshly fallen snow [kg m-3], if densification_method is set to 'constant' (modified, was 300.)

albedo_fresh_snow = 0.80                        # albedo of fresh snow [-] (Moelg et al. 2012, TC) (modified to my data)
albedo_firn = 0.55                              # albedo of firn [-] (Moelg et al. 2012, TC)
albedo_ice = 0.28                               # albedo of ice [-] (Moelg et al. 2012, TC) (modified to my data)
...
albedo_mod_snow_aging = 3                     # effect of ageing on snow albedo [days] (Moelg et al. 2012, TC)
